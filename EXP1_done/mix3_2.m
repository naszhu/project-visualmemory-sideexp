...
if ~exist(filename)
    ...
    %nblocks=input('# oFf blocks for pilot test ');
    %ntrials=input('# of trials for pilot test ');
    nblocks=1;
    ntrials=10;
%     nblocks=7;
%     ntrials=25;%recsiz=input('size of color squares (40) ');
    recsiz=200;
    ...
    %prob(test|test old)
    probchart_old={[0.9,0.1],[0.9,repmat(1/30,1,3)],[0.9,repmat(1/50,1,5)];...
    [1/2,1/2], [repmat(1/4,1,4)],[0.228169,repmat(0.154366,1,5)];...
    [1/2,1/2],[repmat(1/4,1,4)],[0]};
    %prob(test i |test new)
    prob_c_d=[1/3,1/6];
    probchart_new={[prob_c_d,repmat(1/10,1,5)],[prob_c_d,repmat(1/6,1,3)],[prob_c_d,1/2];...
    [prob_c_d,repmat(1/10,1,5)],[prob_c_d,repmat(1/6,1,3)],[prob_c_d,1/2];...
    [prob_c_d,repmat(1/8,1,4)],[1/3,1/6,repmat(1/4,1,2)],[0]};
    %
        ...
        for trial=1:ntrials
            ...
            if flag == 0
                ...
                for i=1:setsize
                 ...
                 %% Added for probtest
                ittime=1:(n-ss);
                for i=ittime %write where ss dind't cover
                    fprintf(fid,'%5d',0);
                end               
                if block==nblocks && trial==ntrials
                    disp('im here');
                    probt=probtestq(wind1,rect);
                    fprintf(fid,'%5d',probt);
                end
                ...
                if block==nblocks && trial==ntrials
                    disp('im here');
                    probt=probtest_instruction(wind1,rect);
                    fprintf(fid,'%5d',probt);
                end
