%%Change by Lea from Rui Cao 9/12/2019
%%
debug=0;
debug_block=1;
synct=0; 
...
subid=input(' subject # ');
if debug==1
    filename=[data_location 'CMatDebug' num2str(subid) '.txt'];
else
    filename=[data_location 'CMat' num2str(subid) '.txt'];
end
consistvars=[data_location 'CMat' num2str(subid)];
 ...
 %randomized the order of the images for all blocks
    ...
    for block=1:nblocks
        ...
        for trial=1:ntrials
            ...
            %%choose memory set
            ms_stimnum=99999999; %each trial have 1 array
            ...
            if Sidechoice==0 %left
                ...
                if old==1 %The response should be learned
                    ...
                    if is.cm==1
                        kstim=order(serpos)+8;%!!!EX:=2;serpos is randomly chosen prviously, this argument give the stimulus num 
