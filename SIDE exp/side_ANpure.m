...
subid=input(' subject # ');
if debug==1
    filename=[data_location 'PureANdebug' num2str(subid) '.txt'];
else
    filename=[data_location 'PureAN' num2str(subid) '.txt'];
end

consistvars=[data_location 'PureAN' num2str(subid)];
 ...
 %randomized the order of the images for all blocks
    ...
    for block=1:nblocks
        ...
        for trial=1:ntrials
            ...
            memsetright={}; 
            memset={};
            ms_stimnum=99999999; %memory set stimulus number
            ...
            for k=1:setsize
                ...
                memset{k}=fullset{antot}; %CHANGE if need; all AN
                ms_stimnum(k)=antot;
            end
            ...
            %probablty it doesn't have to be this lengthy, but we write like this to be consistent. 
            ...
            if flag == 0
                ...
                stimkind_store(tot_trials)=is.cm;  %1: cm; 0:an the current test prob or the current tiral is cm or an 
                %filecondition(tot_trials)=fn;
                
                set_store=9999999999;
                for i=1:setsize
                    %msstim=ms_stimnum(i);
                    %disp('now');disp(class(ms_stimnum(i)));
