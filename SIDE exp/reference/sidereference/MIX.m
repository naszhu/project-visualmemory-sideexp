    ...
    for block=1:nblocks
        ...
        for trial=1:ntrials
            ...
            %% test condition: CM (1) vs. VM (2) 
                ...
                for k=1:(setsize/2) %% set the study set
                    memlist{k}=pictures{order_CM(k)}; %because all the old items are the first 4 from the picture set
                    memlist{k+(setsize/2)}=pictures{order_VM(k)};
                    order_list{k}=order_CM(k);
                    order_list{k+setsize/2}=order_VM(k);   %log the order of the stimuli as well
                end
                ...
                % When it is old test probe,check if the stumi is actually CM or VM
                if old==1
                    ...
                    %kstim=order(serpos); %%%%%%%%%%%%%%%%%check the
                    %difference between the teststim and kstim
                else
