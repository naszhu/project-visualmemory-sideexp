...
consistvars=[data_location 'MiX2' num2str(subid)];
...
if ~exist(filename)
 ...
 %randomized the order of the images for all blocks
    ...
    %%fullset is now a randomized full set, randomized for each partiicpant 
    for i=1:length(images)
        fullset{i}=fullset0{order(i)};
    end


    for i=1:CMtot %4 on each set
        pictures{i}=fullset{i}; 
    end
    for i=1:CMtot/2
        CM_left{i}=pictures{i}; %CM_left and CM_right have the sequence as it is in fullset
        CM_right{i}=pictures{i+CMtot/2};
    end
    ...
    for block=1:nblocks
        ...
        for trial=1:ntrials
            ...
            ordertemp=randperm(CMtot/2); %4 CM in the pool of one side at most, so use 4 is good
            ...
            for k=1:(setsize/2) %you know, half CM, half AN for no matter setsize
                ...
                if Sidechoice==0 %left, only add an ingrediences when test on that side
                    ...
                    antot=antot+1;
                    memsetlefttemp{k+setsize/2}=fullset{CMtot+antot};

                    ms_stimnum(k)=ordertemp(k);
                    ms_stimnum(k+setsize/2)=CMtot+antot;
                else %right
                    memsetrighttemp{k}=CM_right{ordertemp(k)}; %memright_cm
                    antot=antot+1;
                    memsetrighttemp{k+setsize/2}=fullset{CMtot+antot};
                    
                    ms_stimnum(k)=ordertemp(k)+CMtot/2;
                    ms_stimnum(k+setsize/2)=CMtot+antot;
                end
            end
            ...
            %%new-just added
            ms_newstimnum=99999999;
            for k=1:setsize

                ms_newstimnum(k)=ms_stimnum(ordernow(k))
            end
...
%                     disp('serpos:');disp(serpos);disp(class(serpos));
                    %m=ordernow(serpos);
                    kstim=ms_stimnum(ordernow(serpos));%EX:=2;serpos is randomly chosen prviously, this argument give the stimulus num 
                    ...
                    if newca<0.5 % test CM new
                        k=randi(CMtot/2); %4 for cm one side
                        kstim=k+CMtot/2; %because test the other side, so +CMtot
                        teststim=CM_right{k};
                    else
                        antot=antot+1;
                        kstim=CMtot+antot;
            ...
            if flag == 0
                ...
                for i=1:setsize
                    set_store(tot_trials,i)=ms_newstimnum(i); %changed
                end
                %%
